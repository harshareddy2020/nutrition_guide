...
var m = [60, 0, 10, 0],
    w = width - m[1] - m[3],
    h = height - m[0] - m[2],
    xscale = d3.scalePoint().range([0, w]),
    yscale = {},
    dragging = {},
    line = d3.line(),
    axis = d3.axisLeft().tickArguments(1+height/50),
    data,
    foreground,
    background,
    highlighted,
...
d3.csv("../data/nutrients.csv", function(raw_data) {
  ...
  g.append("svg:g")
      .attr("class", "brush")
      .attr("id", function(d){return d.slice(0, -4);})
      .each(function(d) { d3.select(this).call(yscale[d].brush = d3.brush().on("brush", brush)); })
    .selectAll("rect")
...
function brush() {
  brush_count++;
    var actives = dimensions.filter(function(p) {
            return !(!d3.brushSelection(document.getElementById(p.slice(0, -4)))); }),
      extents = actives.map(function(p) {
        var tempr = d3.brushSelection(document.getElementById(p.slice(0, -4)));
        var invertingScale = d3.scaleLinear().domain(yscale[p].range()).range(yscale[p].domain());
        return [invertingScale(tempr[1][1]), invertingScale(tempr[0][1])];});
...
//TODO
  ...
  // free text search
  var query = d3.select("#search")['_groups'][0][0].value;
  if (query.length > 0) {
...
function update_ticks(d, extent) {
  ...
  if (d) {
    ...
    if (extent) {
      ...
      brush_el.call(yscale[d].brush = d3.brush().extent(extent).on("brush", brush));
    } else {
      brush_el.call(yscale[d].brush = d3.brush().on("brush", brush));
    }
  } else {
    // all ticks
    d3.selectAll(".brush")
...
window.onresize = function() {
  ...
  xscale = d3.scalePoint().range([0, w], 1).domain(dimensions);
